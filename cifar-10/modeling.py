...
class PixelAttention(nn.Module):
    def __init__(self, input_version, color, use_cls, embedding_dim, hidden_dim1, hidden_dim2, n_layers, n_attn_heads, n_classes, dropout,
                 position_embedding_version, segment_embedding_version, attention_version, attention_indv_channels_merge_mode):
        super(PixelAttention, self).__init__()
        self.input_version, self.color = input_version, color
        if "square" in self.input_version:
            seq_len = int(2*embedding_dim/int(input_version[input_version.find("_") + 1:]))
            embedding_dim = int(input_version[input_version.find("_") + 1:]) ** 2
        self.use_cls = True if use_cls == "True" else False
        ...
        if hidden_dim1 != embedding_dim:
            print("The hidden dimension on the attention is different than the embedding dimension... "
                  "This is not allowed, so setting hidden_dim = embedding_dim")
        ...
        if not self.use_cls:
            if "square" in self.input_version:
                self.flat = nn.Linear(seq_len*hidden_dim1*embedding_dim_mult, embedding_dim*embedding_dim_mult)
            else:
                if self.input_version == "rows":
                    self.flat = nn.Linear(embedding_dim*hidden_dim1*embedding_dim_mult, embedding_dim*embedding_dim_mult)
                elif self.input_version == "rows_and_columns_cat":
                    self.flat = nn.Linear(2*embedding_dim*hidden_dim1*embedding_dim_mult, embedding_dim*embedding_dim_mult)
    ...
    def forward(self, x, position_ids=None, segment_ids=None):
        ...
        if self.input_version == "rows_and_columns_cat" or self.input_version == "rows":
            if self.color:
                if self.attention_version == "default":
                    if self.input_version == "rows_and_columns_cat":
                        if self.use_cls:
                ...
                n_examples, seq_len, x_device = len(x), x.shape[1], x.device
        elif "square" in self.input_version:
            square_dim, n_examples = int(self.input_version[self.input_version.find("_")+1:]), len(x)
            se_len = int(2*x.shape[1]/square_dim)
            seq_len_half = int(se_len/2)
            color_mult = 3 if self.color else 1
            x_prep = torch.zeros((len(x), int(2*seq_len_half), square_dim**2*color_mult)).to(x.device)
            for h in range(seq_len_half):
                for w in range(seq_len_half):
                    x_prep[:, h:h+1, :].copy_(
                        x[:, h*square_dim:(h+1)*square_dim, w*square_dim:(w+1)*square_dim].reshape(n_examples, 1, -1).data
                    )
            x = x_prep
